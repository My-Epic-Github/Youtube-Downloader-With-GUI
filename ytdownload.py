...
layout = [[sg.Titlebar('Basic YouTube Downloader With GUI')],
    [sg.Text('Video URL:'),sg.Input(key='-INPUT-'), sg.DropDown(['144p','240p', '360p','720p', '1080p', '1440p', '2160p' ], key='-RES-' )],
        [sg.Button('Download', key='Download'), sg.Checkbox('Download Audio Only', key='box', enable_events=True, default=False)]
...
window = sg.Window('YouTube Downloader', layout, alpha_channel=5 )
...
while True:
    try:
        ...
        if event == 'Download' and values['-RES-'] == '144p' or '240p' or '360p' or '480p' or '720p' or '1080p' or '1440p' or '2160p':
            ...
            aud = yt.streams.filter(only_aiudio=True).first().download(output_path='C:\Temp', filename='Aud.mp4')
        ...
        if event =='Download' and 'box' == True:
            
            input_value = values['-INPUT-']
            yt = YouTube(input_value)
            yt.streams.filter(only_audio=True).first().download(output_path=f'{desk}\Youtube', filename='VidWithAudioOnly.mp3')
        if values['box'] == True:
