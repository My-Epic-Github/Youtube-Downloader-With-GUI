...
font2 = ("Comic Sans", 12,)
resfont = ('Comic Sans', 12, 'underline')
...
layout = [
    [sg.Text('Video URL:', font=font2),sg.Input(key='-INPUT-'), sg.DropDown(['144p','240p', '360p','720p', '1080p', '1440p', '2160p', '4320p' ], key='-RES-', font=resfont)],
        [sg.Button('Download', key='Download', font=font2)],
        ]

iconlol = output.YtIcon
window = sg.Window('YouTube Downloader', layout, resizable=False, use_custom_titlebar=True, titlebar_icon=iconlol, titlebar_font=font)
...
while True:
    ...
    try:
        ...
        if event == '-INPUT-':
            window['-T2-'].update(visible=True)
            window['-T2-'].update(f'Video Info:\n{yt.title}\n{yt.views}\n{yt.channel_url}')
